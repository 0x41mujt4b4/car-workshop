...
import { useNavigate } from "react-router-dom";
  ...
  return (
    ...
    <div className="flex flex-row gap-2 justify-between md:px-20 lg:px-28 pb-4">
      ...
      <div className="inline-grid w-full overflow-y-auto max-h-80 md:px-20 lg:px-28">
        <table className="table text-center">
          <thead className="bg-cyan-700 text-white top-0 sticky ">
            <tr>
              {tableHead.map((el, key) => (
                <th className={style.th} key={key}>{el}</th>
              ))}
            </tr>
          </thead>
          <tbody>{isLoading ? null : products ? (
              products.map((product, key) => (<tr
                  key={key}
                  className="bg-white rounded-[20px] active:bg-slate-200 border border-b-black"
                ><td
                    className={style.td}
                    // onClick={() => addProductToCard(product)}
                  >{product.repair_cost}</td>
                  <td
                    className={style.td}
                    // onClick={() => addProductToCard(product)}
                  >{product.total_cost}</td>
                  <td
                    className={style.td}
                    // onClick={() => addProductToCard(product)}
                  >{product.color}</td>
                  ...
                  <td
                    className={style.td}
                    // onClick={() => addProductToCard(product)}
                  >{product.plate_no}</td></tr>))) : (<h2 className="text-red-300">لا يوجد منتج</h2>)}</tbody>
        </table>
      </div>
    </div>
  </div>
